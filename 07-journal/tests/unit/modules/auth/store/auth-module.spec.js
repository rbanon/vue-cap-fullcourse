...
describe("Vuex: Pruebas en el auth-module", () => {
  ...
  test("Mutation: loginUser", () => {
    ...
    const payload = {
      user: { name: "Rafa", email: "rafa@gmail.com" },
      idToken: "ABC-123",
    ...
    expect(user).toEqual({ name: "Rafa", email: "rafa@gmail.com" });
  ...
  test("Mutation: logout", () => {
    ...
    const store = createVuexStore({
      ...
      user: { name: "rafa", email: "fer@gmail.com" },
      idToken: "ABC-123",
  ...
  test("Getter: username currentState", () => {
    ...
    expect(store.getters["auth/username"]).toBe("rafa");
  });
  ...
  test("Actions: createUser signInUser - Crea el usuario", async () => {
    ...
    const deleteResp = await axios.post(
      `https://identitytoolkit.googleapis.com/v1/accounts:delete?key=AIzaSyBhCquFEBH1rgR5IJFo1v1qGiZzHDe2PVI`,
      {
        idToken,
      }
    );
